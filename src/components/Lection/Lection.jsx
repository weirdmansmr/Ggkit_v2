...
const Lection = () => {
	...
	const lection = `**Информатика** – это дисциплина, изучающая структуру и общие свойства информации, закономерности и методы её создания, хранения, поиска, преобразования, передачи и применения в различных сферах человеческой деятельности.
...
`;
	...
	// useEffect(() => {
	// 	fetch(MARKDOWN).then(res => res.text()).then(md => setMark(md))
	// }, [])

	return (
		<div className={style.markCont}>
			<Markdown children={lection} />
		</div>
	);
};
